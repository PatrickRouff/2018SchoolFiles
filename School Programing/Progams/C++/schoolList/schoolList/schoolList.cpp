...
highschool* search(string h, int &x)
{
	...
	if (!found) {
		return NULL;
			...
			// Me - after function complete, NULL will only be returned if not found
...
void menu()
{
	...
	case 3:							// Student needs to add some statements for case 3
		if (headptr == NULL) {							//if the list is empty
		...
		else //... perform the search
		{
			...
			if (r != NULL)//if r points to a highschool on the list
			{
				cout << "School found at position: " << pos << endl;
				cout << left << pos << ". " << setw(33) << curptr->sname << setw(25) << curptr->mascot << setw(5) << curptr->fyear << endl;//cout<<pos<<". "<<setw(33)... finish this!
			}
			else {
